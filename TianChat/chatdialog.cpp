    ...
    ,_b_loading(false)
{
    ...
    //ui->side_head_lb->setPixmap(pixmap);    //把图片加载到label上
    QPixmap scaledPixmap=pixmap.scaled(ui->side_head_lb->size(),Qt::KeepAspectRatio);
    ui->side_head_lb->setPixmap(scaledPixmap);  //将放缩后的图片设置到label上
    ui->side_head_lb->setScaledContents(true);  //设置label自动缩放内容以适应大小
